...
class MPCController:
    ...
    def reset_warm_start(self) -> None:
        """Reset controller memory at the start of a new independent rollout/attempt.

        This intentionally does NOT disable step-to-step MPC warm-start inside a rollout.
        It only clears any stored solution/control before a fresh closed-loop simulation.
        """
    ...
    def solve(
        ...
        # Step-to-step warm-start inside the SAME closed-loop rollout is intentionally kept.
        # Independent hardware attempts should create a fresh MPCController or call reset_warm_start().
        # IPOPT warm_start_init_point='no' only disables IPOPT internal warm-start mode;
        # the shifted previous solution below is still a normal MPC initial guess.
        guess_kind = 'default'
        x0_guess = None
        if self.p.warmstart_shift:
            x0_guess = self._shift_last_solution(current_x)
            if x0_guess is not None:
                guess_kind = 'shifted_prev_solution'
        elif self.last_solution_vec is not None:
            x0_guess = np.asarray(self.last_solution_vec, dtype=float).copy()
            guess_kind = 'raw_prev_solution'
        ...
        try:
            ...
            if is_success:
                self.last_solution_vec = sol_vec.copy()
            u_out = sol_vec[n_X:n_X + self.model.n_controls]
        except Exception:
            u_out = self.last_u.copy()
